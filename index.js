...
app.post('/api/analyser', async (req, res) => {
    ...
    try {
        // 1. Analyse IA avec instructions de traduction RENFORCÉES
        const completion = await openai.chat.completions.create({
            ...
            messages: [
                {
                    role: "system",
                    content: "Tu es un expert Pokémon TCG et un traducteur Français-Anglais intraitable. Analyse l'image et le titre. Retourne un JSON pur : {'nom_anglais': 'nom EN ANGLAIS EXACT', 'numero': 'uniquement les chiffres', 'set_nom': 'nom de l\'extension'}. Fais TRÈS ATTENTION aux évolutions (ex: Ptitard=Poliwag, Têtarte=Poliwhirl, Reptincel=Charmeleon)."
                },
                {
                    role: "user",
                    content: [
                        { type: "text", text: `Titre Vinted : ${title}. Traduis le nom du Pokémon en anglais avec une précision absolue. Isole le numéro (ex: pour 'MEW 176', le numero est '176').` },
